...
function emptyDashboard(businessName = "Your operator"): OperatorDashboardData {
  return {
    businessName,
    stats: [
      {
...
function emptyVerification(
  ...
  return {
    ...
    panNumber: null,
    gstNumber: null,
    mtdcRegistrationNo: null,
    insuranceStatus: false,
    insuranceProvider: null,
    insuranceDetails: null,
    femaleGuideCount: 0,
    totalGuideCount: 0,
    posHPolicyStatus: "NOT_STARTED",
...
export async function getOperatorVerification(
  ...
  return withDbFallback(
    () =>
      prisma.operator.findUniqueOrThrow({
        where: { id: operatorId },
        select: {
          businessName: true,
          yearsOperating: true,
